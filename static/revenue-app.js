// 收益测算页前端 —— 使用 window.AgentCommon 共享工具
(() => {
  ...
  async function loadModel(forceDefaults = false) {
    ...
    if (!els.paramGrid.children.length) els.paramGrid.innerHTML = skeleton(6);
    try {
  ...
  async function init() {
    ...
    mountBackToTop();
    // Ctrl/Cmd+P 触发浏览器打印，由 shared.css 的 @media print 处理
    await loadParamHistory();
    await loadModel();
  }
